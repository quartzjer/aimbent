...
@dataclass
class AudioConfig:
    format: str = 's16le'  # 16-bit PCM little-endian
    channels: int = 1
    rate: int = 24000
    layout: str = 'mono'
...
class PCMFileWriter:
    def __init__(self, filename: str = './recording.pcm', config: AudioConfig = AudioConfig()):
        self.config = config
        ...
        self.file: BinaryIO = open(filename, 'wb')
        
        # Start writer thread
        self.writer_thread = threading.Thread(target=self._writer_loop, daemon=True)
        self.writer_thread.start()
        logger.info(f"Started PCM recording to {filename}")
    ...
    def close(self) -> None:
        logger.info("Closing PCM file writer")
        self.running = False
...
class RTMPAudioController(SimpleRTMPController):
    ...
    async def on_ns_publish(self, session: SessionManager, message) -> None:
        session.state = PCMFileWriter()  # Initialize PCM writer instead of audio player
